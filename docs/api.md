...
Tokenize text using a specific model's tokenizer. This endpoint converts text into a list of token IDs that the model uses internally.

### Parameters

- `model`: (required) the [model name](#model-names) to use for tokenization
- `content`: (required) the text to tokenize
- `media_type`: (optional) the type of content being tokenized (default: `"text"`). Future-proofs for multimodal tokenization (e.g., image/audio/text)

Advanced parameters:

- `options`: additional model parameters listed in the documentation for the [Modelfile](./modelfile.md#valid-parameters-and-values)
- `keep_alive`: controls how long the model will stay loaded into memory following the request (default: `5m`)
...
{
  "model": "llama3.2:3b",
  "tokens": [1, 15043, 29871, 29941],
  "total_duration": 1234567,
  "load_duration": 987654
}
```
...
Convert a list of token IDs back to text using a specific model's tokenizer. This is the reverse operation of tokenization.

### Parameters

- `model`: (required) the [model name](#model-names) to use for detokenization
- `tokens`: (required) the list of token IDs to detokenize
- `media_type`: (optional) the type of content being detokenized (default: `"text"`). Future-proofs for multimodal tokenization (e.g., image/audio/text)
